...
class TavusSimpleService {
  ...
  constructor() {
    ...
    if (__DEV__) {
      console.log('🔧 Tavus Simple Service Configuration:');
      ...
      console.log('🎯 No WebSocket - Using webhook-based transcript processing');
    }
  }
...
CRITICAL: Use their name "${metadata.user_full_name || metadata.user_name || 'the pet parent'}" frequently - NEVER say "dog parent" or generic terms. Follow the Paw-some Progress Challenge hook and 3C Method from your training. Make this personal, engaging, and interactive!`;
      ...
      console.log('✅ Tavus LIVE conversation created successfully:', {
        ...
        webhook_url: ApiConfig.TAVUS.WEBHOOK_URL,
      ...
      if (sessionId && !sessionId.startsWith('mock_') && !ApiConfig.TAVUS.USE_MOCK_MODE) {
        try {
          ...
          if (!response.ok) {
            ...
            console.log('🎯 Transcript will be processed by webhook automatically');
          }
        } catch (apiError) {
  ...
  /**
   * No WebSocket connection - using webhook processing
   */
  isConnected(): boolean {
    return false; // We don't use WebSocket anymore
  }
}

// Export singleton instance
export const tavusSimpleService = new TavusSimpleService(); 
