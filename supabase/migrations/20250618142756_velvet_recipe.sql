/*
  # VetPaw AI Coaching System Database Schema

  1. New Tables
    - `coaching_sessions` - Main coaching session records
    - `coaching_messages` - Real-time transcript storage  
    - `session_summaries` - AI-generated session analysis

  2. Security
    - Enable RLS on all coaching tables
    - Add policies for users to manage their own coaching data
...
DO $$
BEGIN
    -- Drop coaching_sessions policies
    ...
    -- Drop coaching_messages policies
    DROP POLICY IF EXISTS "Users can view own coaching messages" ON coaching_messages;
    DROP POLICY IF EXISTS "Users can insert own coaching messages" ON coaching_messages;
    DROP POLICY IF EXISTS "Users can update own coaching messages" ON coaching_messages;
    DROP POLICY IF EXISTS "Users can delete own coaching messages" ON coaching_messages;
    
    -- Drop session_summaries policies
    DROP POLICY IF EXISTS "Users can view own session summaries" ON session_summaries;
    ...
    DROP POLICY IF EXISTS "Users can delete own session summaries" ON session_summaries;
EXCEPTION
    WHEN OTHERS THEN
...
-- Add trigger to update updated_at column (only if function exists)
DO $$
BEGIN
